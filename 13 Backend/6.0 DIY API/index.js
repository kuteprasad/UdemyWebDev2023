...
app.get("/random", (req, res) => {
  res.json(jokes[generateRandomNumber(jokes.length)]);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  // jokes.push(data);
  const newIndex = jokes.findIndex((joke) => joke.id === data.id);
  // updateJoke.jokeText = data.jokeText;
  // updateJoke.jokeType = data.jokeType;
  jokes[newIndex] = data;
  // console.log(jokes.slice(-1));
  res.json(jokes[newIndex]);
});
//6. PATCH a joke
app.patch("/jokes/:id", (req, res) => {
  ...
  const newdata = {
    id: id,
    jokeText: req.query.text || oldJoke.jokeText,
    jokeType: req.query.type || oldJoke.jokeType,
  };

  const searchId = jokes.findIndex((joke) => joke.id === id);

  jokes[searchId] = newdata;
...
app.delete("/jokes/all", (req, res) => {
  ...
  if (userkey === masterKey) {
    ...
    res.statusCode(404).json("Please provide correct Master Key");
  }
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  var response = removeItemOnce(jokes, searchId);

  if (response) {
    res.json("Deleted");
  } else {
    res.json("Array out of lower limit");
  }
});
